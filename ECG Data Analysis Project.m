...
% Here we just use the FIRST 10 seconds. If needed, change the start time.
...
% Use a small Resting window (e.g. 5 seconds) for demonstration
...
% Default-ish findpeaks (no special parameters)
...
% Very simple height-based threshold, scaled by the max amplitude
...
% Use a fixed window around each R peak (in samples)
...
% Use a specific 10-second window (you can adjust these times)
% If your recording is shorter, change 120 and 130 to fit your data.
...
% Test 1: Rest vs Exercise HR (same 120-130 s window)
...
% Window B: 10 seconds later (t_start+20 to t_end+20) - adjust if needed
...
% Simple helper to collect aligned beats around each R peak
